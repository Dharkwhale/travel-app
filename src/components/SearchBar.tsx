...
import type { Activity, Hotel } from "../constants/interface";
...
const SearchBar = ({
  ...
  const [filteredResults, setFilteredResults] = useState<(FlightData | Activity | Hotel)[]>([]);
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const activityResults = activitiesData.filter((activity) => {
      const { name, shortDescription, representativePrice } =
        activity;
      return (
        name.toLowerCase().includes(query.toLowerCase()) ||
  ...
  const handleAddRemove = (
    item: FlightData | Activity | Hotel,
    type: "flight" | "activity" | "hotel"
  ) => {
    if (type === "flight") {
      const addedItems: FlightData[] = JSON.parse(localStorage.getItem("addedFlights") || "[]");
      const index = addedItems.findIndex((i) => i.iataCode === (item as FlightData).iataCode);
      if (index === -1) {
        addedItems.push(item as FlightData);
      } else {
        addedItems.splice(index, 1);
      }
      localStorage.setItem("addedFlights", JSON.stringify(addedItems));
      setAddedFlights(addedItems);
      onFlightAddRemove(item as FlightData);
    } else if (type === "activity") {
      const addedItems: Activity[] = JSON.parse(localStorage.getItem("addedActivities") || "[]");
      const index = addedItems.findIndex((i) => i.id === (item as Activity).id);
      if (index === -1) {
        addedItems.push(item as Activity);
      } else {
        addedItems.splice(index, 1);
      }
      localStorage.setItem("addedActivities", JSON.stringify(addedItems));
      setAddedActivities(addedItems);
      onActivityAddRemove(item as Activity);
    } else if (type === "hotel") {
      const addedItems: Hotel[] = JSON.parse(localStorage.getItem("addedHotels") || "[]");
      const index = addedItems.findIndex((i) => i.hotel_id === (item as Hotel).hotel_id);
      if (index === -1) {
        addedItems.push(item as Hotel);
      } else {
        addedItems.splice(index, 1);
      }
      localStorage.setItem("addedHotels", JSON.stringify(addedItems));
      setAddedHotels(addedItems);
      onHotelAddRemove(item as Hotel);
    }
  };
    ...
    <div className="relative w-full max-w-md mx-auto">
      ...
      {!loading && filteredResults.length > 0 && dropdownVisible && (
        ...
        >
          <FilterOptions />
          <div className="p-4">
            {filteredResults.map((item: FlightData | Activity | Hotel) => {
              if ("iataCode" in item) {
                return (
                  <div
                    key={item.iataCode}
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2 items-end">
                      ...
                      <button
                        className={`py-1 text-white text-[10px] rounded-md flex items-center justify-center gap-1 ${
                      ...
                      >
                        {addedFlights.some(
                          ...
                          <PiMinus size={12} className="text-white"/>
                        ) : (
                          <PiPlus size={12} />
                        )}
                        {addedFlights.some((f) => f.iataCode === item.iataCode)
